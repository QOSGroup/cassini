...
>>>>>>> 升级qbase,qos依赖版本
		...
		if len(r.URL.Path) > 1 {
			WriteRPCResponseHTTP(w, types.RPCInvalidRequestError(
				request.ID, errors.Errorf("Path %s is invalid", r.URL.Path)))
			return
		}
		...
		if len(request.Params) > 0 {
			...
			if err != nil {
				WriteRPCResponseHTTP(w,
					types.RPCInvalidParamsError(request.ID,
...
func nonJSONStringToArg(cdc *amino.Codec, rt reflect.Type, arg string) (reflect.Value, bool, error) {
	if rt.Kind() == reflect.Ptr {
		rval, ok, err := nonJSONStringToArg(cdc, rt.Elem(), arg)
		if err != nil {
...
func _nonJSONStringToArg(cdc *amino.Codec, rt reflect.Type, arg string) (reflect.Value, bool, error) {
	isIntString := RegexpInt.Match([]byte(arg))
...
func (wsc *wsConnection) readRoutine() {
	...
	for {
		...
		default:
			...
			log.Info("WSJSONRPC", "method", request.Method)
...
func (wsc *wsConnection) writeRoutine() {
	...
	defer func() {
		...
		if err := wsc.baseConn.Close(); err != nil {
			log.Error("Error closing connection", "err", err)
		}
	}()
	...
	for {
		...
		case m := <-pongs:
			...
			if err != nil {
				log.Info("Failed to write pong (client may disconnect)", "err", err)
			}
		case <-pingTicker.C:
			err := wsc.writeMessageWithDeadline(websocket.PingMessage, []byte{})
			if err != nil {
				log.Error("Failed to write ping", "err", err)
				wsc.Stop()
				return
			}
		case msg := <-wsc.writeChan:
			...
			if err != nil {
				log.Error("Failed to marshal RPCResponse to JSON", "err", err)
			} else {
...
func (wm *WebsocketManager) WebsocketHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		log.Error("Failed to upgrade to websocket connection", "err", err)
		return
	}
	...
	if err != nil {
		log.Error("Error starting connection", "err", err)
	}
}
